...
use \JsonRpcLib\Server\Input\Data\Memory as InputData;
use \JsonRpcLib\Server\Output\Data\Memory as OutputData;
...
class ServerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $in = new InputData($value);
        $out = new OutputData();

        $inputMessage = new InputMessage($in);
        $outputMessage = new OutputMessage($out);

        $manager = new Manager();

        $server = new Server($manager);

        $server->addService(new ClosureWrapper($this->closureSum), 'sum');
